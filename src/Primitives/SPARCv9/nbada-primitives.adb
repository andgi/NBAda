...
--  $Id: nbada-primitives.adb,v 1.13 2007/04/19 12:16:16 andersg Exp $
-------------------------------------------------------------------------------
...
package body Primitives is
   ...
   ----------------------------------------------------------------------------
   --  Synchronization primitives for 64-bit SPARC v9.
   --
   --  Requires the flags '-m64 --RTS=m64 -cargs -Wa,-xarch=v9' to gnatmake.
   ----------------------------------------------------------------------------
   ...
   begin
      System.Machine_Code.Asm
        (Template =>
           ...
           "membar #LoadLoad | #StoreStore | #LoadStore | #StoreLoad" &LF &HT&
           "mov %2, %%l0"                & LF & HT &   -- l0 <- %3
           "casx [%0], %1, %%l0"         & LF & HT &   -- Compare & swap
           "membar #LoadLoad | #StoreStore | #LoadStore | #StoreLoad" &LF &HT&
           "!#END Compare_And_Swap_64",
         Inputs   => (Element_Access'Asm_Input ("r",       -- %0 = Target
                                                Element_Access (Target)),
   ...
   begin
      System.Machine_Code.Asm
        (Template =>
           "!#BEGIN Fetch_And_Add_64"      & LF &
           "membar #LoadLoad | #StoreStore | #LoadStore | #StoreLoad" &LF &HT&
           "retry3:"                       & LF & HT &   -- retry:
           "ldx [%0], %%l0"                & LF & HT &   --  l0 <- [%0]
           "add %%l0, %1, %%l1"            & LF & HT &   --  l1 <- l0 + %1
           "casx [%0], %%l0, %%l1"         & LF & HT &   --  cas [%0] l0 l1
           "cmp %%l0, %%l1"                & LF & HT &   --  if l0 /= l1
           "bne retry3"                    & LF & HT &   --   goto retry
           "nop"                           & LF & HT &
           "membar #LoadLoad | #StoreStore | #LoadStore | #StoreLoad" &LF &HT&
   ...
   begin
      System.Machine_Code.Asm
        (Template =>
           ...
           "ldx [%1], %%l0"                & LF & HT &   --  l0 <- [%1]
           "add %%l0, %2, %%l1"            & LF & HT &   --  l1 <- l0 + %2
           "casx [%1], %%l0, %%l1"         & LF & HT &   --  cas [%1] l0 l1
           "cmp %%l0, %%l1"                & LF & HT &   --  if l0 /= l1
           "bne retry4"                    & LF & HT &   --   goto retry
           "nop"                           & LF & HT &
           "mov %%l1, %0"                  & LF & HT &   --  %0 <- l1
           "membar #LoadLoad | #StoreStore | #LoadStore | #StoreLoad" &LF &HT&
           "!#END Fetch_And_Add_64",
