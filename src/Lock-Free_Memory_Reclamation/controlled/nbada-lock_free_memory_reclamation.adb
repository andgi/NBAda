...
package body Lock_Free_Memory_Reclamation is
   ...
   Local_Reference_Count : array (Processes, HP_Index) of Natural
     := (others => (others => 0));
   --  Used to keep track of the number of local references associated with
   --  each hazard pointer.
   ...
   package body Operations is
      ...
      ----------------------------------------------------------------------
      function To_Private_Reference_Impl is
      ...
      begin
         ...
         if not Found then
            ...
            loop
               Node_Ref.Ref := To_Private_Reference_Impl (Link.all);
               ...
               --  Link is read again.
               exit when To_Private_Reference_Impl (Link.all) = Node_Ref.Ref;
            end loop;
         end if;
         if To_Node_Access (Node_Ref) /= null then
            Local_Reference_Count (ID, Index) := 1;
            return Node_Ref;
         else
      ...
      begin
         ...
         for I in Hazard_Pointer'Range (2) loop
            ...
            then
               Local_Reference_Count (ID, I) :=
                 Local_Reference_Count (ID, I) - 1;
               if Local_Reference_Count (ID, I) = 0 then
                  Hazard_Pointer (ID, I) := null;
               end if;
               exit;
            end if;
         end loop;
         --  Clear the Private_Reference.
         declare
            type Mutable_Access is access all Private_Reference;
            type Immutable_Access is access constant Private_Reference;
            function To_Mutable is new Ada.Unchecked_Conversion
              (Immutable_Access, Mutable_Access);
            Mutable_Ref : constant Mutable_Access :=
              To_Mutable (Node'Access);
         begin
            Mutable_Ref.Ref := 0;
         end;
      end Release;
      ...
      procedure Delete  (Node : in Private_Reference) is
         ...
         ID        : constant Processes := Process_Ids.Process_ID;
         Index     : Node_Index;
         The_Node  : constant Node_Access := To_Node_Access (Node);
      begin
         Release (Node);
         --  Note: The reference Node is now cleared and cannot be
         --        dereferenced anymore.

         declare
            Node_Base : constant Managed_Node_Access :=
              Managed_Node_Access (The_Node);
         ...
         DL_Nodes (ID, Index) := Atomic_Node_Access (The_Node);
      ...
      procedure Store   (Link : access Shared_Reference;
         ...
         Old : constant Node_Access :=
           To_Node_Access (Private_Reference'(Ada.Finalization.Controlled with
                                              To_Private_Reference_Impl
                                              (Link.all)));
      begin
      ...
      begin
         ...
         return Private_Reference'(Ada.Finalization.Controlled with
                                   To_Private_Reference_Impl (Node));
      end Create;
      ...
      function  Is_Marked (Node : in     Shared_Reference)
                          return Boolean is
      begin
         return (To_Private_Reference_Impl (Node) and Mark_Mask) = 1;
      end Is_Marked;
      ...
      begin
         return To_Private_Reference_Impl (Link) = Ref.Ref;
      end "=";
      ...
      ----------------------------------------------------------------------
      procedure Initialize (Ref : in out Private_Reference) is
      begin
         Ref.Ref := 0;
      end Initialize;

      ----------------------------------------------------------------------
      procedure Adjust   (Ref : in out Private_Reference) is
         ID : constant Processes := Process_Ids.Process_ID;
      begin
         --  Find and increase the reference count of the hazard pointer.
         for I in Hazard_Pointer'Range (2) loop
            if
              Hazard_Pointer (ID, I) =
              Atomic_Node_Access (To_Node_Access (Ref))
            then
               Local_Reference_Count (ID, I) :=
                 Local_Reference_Count (ID, I) + 1;
               exit;
      ...
      ----------------------------------------------------------------------
      procedure Finalize (Ref : in out Private_Reference) is
         ID : constant Processes := Process_Ids.Process_ID;
      begin
         --  Find and reduce the reference count of the hazard pointer.
         --  If we have dereferenced the same node several time, there are
         --  several hazard pointers to it. Release removes one.
         if To_Node_Access (Ref) /= null then
            for I in Hazard_Pointer'Range (2) loop
               if
                 Hazard_Pointer (ID, I) =
                 Atomic_Node_Access (To_Node_Access (Ref))
