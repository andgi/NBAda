...
package Test_Dictionaries is
   ...
   package Dictionaries is new
     NBAda.Lock_Free_Dictionaries (Value_Type  => Value_Type,
                                   Key_Type    => Key_Type,
                                   Hash        => Hash,
                                   "<"         => "<",
   ...
   --  Dictionary.
   ----------------------------------------------------------------------------
   subtype Dictionary_Type is Dictionaries.Dictionary_Type (No_Buckets => 100);

   Not_Found       : exception
     renames Dictionaries.Not_Found;
   Already_Present : exception
     renames Dictionaries.Already_Present;

   procedure Init    (Dictionary : in out Dictionary_Type)
     renames Dictionaries.Init;

   procedure Insert  (Into  : in out Dictionary_Type;
                      Key   : in     Key_Type;
                      Value : in     Value_Type)
     renames Dictionaries.Insert;

   procedure Delete  (From : in out Dictionary_Type;
                      Key  : in     Key_Type)
     renames Dictionaries.Delete;

   function  Lookup  (From : in Dictionary_Type;
                      Key  : in Key_Type)
                     return Value_Type
     renames Dictionaries.Lookup;
